    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8 animate-fade-in">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 border-b border-gray-700 pb-4 gap-4">
          <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold flex items-center gap-2"><Shield className="text-purple-500" /> Admin WyRunner</h1>
                <div className="flex items-center gap-2 text-sm mt-1">
                    Estado del Backend:
                    {isFirebaseActive ? (
                        <span className="text-green-400 flex items-center gap-1 font-bold"><CheckCircle size={14} /> Firebase (Cloud)</span>
                    ) : (
                        <span className="text-orange-400 flex items-center gap-1 font-bold"><Database size={14} /> LocalStorage (Demo)</span>
                    )}
                </div>
              </div>
          </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 relative overflow-hidden group">
              ...
              <div className="flex justify-between mb-2 text-gray-400 font-medium">Usuarios Registrados</div>
              <div className="text-4xl font-bold text-white">{stats.totalUsers}</div>
              <div className="text-xs text-gray-500 mt-2">Total en la base de datos</div>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 relative overflow-hidden group">
              <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><DollarSign size={64} /></div>
              <div className="flex justify-between mb-2 text-gray-400 font-medium">Ingresos (Simulados)</div>
              <div className="text-4xl font-bold text-green-400">${stats.revenue}</div>
              <div className="text-xs text-gray-500 mt-2">Basado en créditos</div>
          </div>
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 relative overflow-hidden group">
               <div className="absolute right-0 top-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Database size={64} /></div>
               <div className="flex justify-between mb-2 text-gray-400 font-medium">Gen. de Hoy</div>
              <div className="text-4xl font-bold text-blue-400">{stats.generationsToday}</div>
              <div className="text-xs text-gray-500 mt-2">Documentos creados</div>
          </div>
      </div>
      ...
      <div className="grid md:grid-cols-1 gap-8">
          ...
          <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
              <h3 className="font-bold mb-4 flex items-center gap-2 text-lg"><Bell className="text-yellow-500" size={20} /> Enviar una Notificación</h3>
              <p className="text-sm text-gray-400 mb-4">Envía un mensaje que aparecerá en el dashboard de todos los usuarios (vía Firebase Realtime Database).</p>
              <div className="space-y-3">
                <input 
                    ...
                    placeholder="Mensaje del sistema (ej: Mantenimiento programado...)" 
                />
                <button 
                    onClick={() => { backend.sendNotification(notifMsg); setNotifMsg(''); alert('Notificación enviada'); }}
