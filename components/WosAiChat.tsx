    ...
    <div className="flex flex-col h-[100dvh] bg-white dark:bg-gray-900 text-gray-900 dark:text-white fixed inset-0 z-50">
        ...
        <div className="p-4 border-b dark:border-gray-800 flex items-center gap-3 shadow-sm bg-white dark:bg-gray-900 z-10 shrink-0">
            ...
            <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full wos-gradient flex items-center justify-center text-white shadow-md">
                    <Sparkles size={16} />
                </div>
                <div>
            ...
            {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`flex gap-3 max-w-[85%] md:max-w-[70%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-gray-200 dark:bg-gray-700' : 'wos-gradient text-white'}`}>
                            {msg.role === 'user' ? <User size={16} /> : <Bot size={16} />}
                        </div>
                        <div className={`p-3 rounded-2xl ${
                            msg.role === 'user' 
                            ? 'bg-purple-600 text-white rounded-tr-none' 
                            : 'bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-tl-none shadow-sm'
                        }`}>
            ...
            {isLoading && (
                 <div className="flex justify-start">
                    <div className="flex gap-3 max-w-[85%]">
                        <div className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center wos-gradient text-white">
                            <Bot size={16} />
                        </div>
                        <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 p-3 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-1">
        ...
        <div className="p-4 bg-white dark:bg-gray-900 border-t dark:border-gray-800 shrink-0 safe-area-bottom">
            <div className="flex items-center gap-2 max-w-4xl mx-auto bg-gray-100 dark:bg-gray-800 p-2 rounded-full border dark:border-gray-700 focus-within:ring-2 focus-within:ring-purple-500 transition-all">
                ...
                <button 
                    ...
                    className="p-3 wos-gradient text-white rounded-full hover:shadow-lg disabled:opacity-50 disabled:hover:shadow-none transition"
                >
                    <Send size={18} />
                </button>
            </div>
